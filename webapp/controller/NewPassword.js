sap.ui.define([
	...
	return BaseController.extend("com.bozankaya.ZBZ_MALZEME_NAKIL.controller.NewPassword", {

		_getDialog: function() {
			if (!this.oNewPassword) {
				this.oNewPassword = sap.ui.xmlfragment("com.bozankaya.ZBZ_MALZEME_NAKIL.view.NewPassword", this);
			}
		...
		onSaveNewPass: function() {
			...
			if (!newPass1Value || !newPass2Value || !currrentPass) {
				sErrMsg = this.getView().getModel("i18n").getResourceBundle().getText("fillPassValues");
			...
			if (currrentPass !== this.aData.Password) {
				sErrMsg = this.getView().getModel("i18n").getResourceBundle().getText("checkCurrPass");
				sap.ui.getCore().byId("newPassWarningMsg").setText(sErrMsg);
				sap.ui.getCore().byId("newPassWarningMsg").setVisible(true);
				return;
			}
			...
			if (newPass1Value !== newPass2Value) {
				sErrMsg = this.getView().getModel("i18n").getResourceBundle().getText("nePassValues");
				sap.ui.getCore().byId("newPassWarningMsg").setText(sErrMsg);
				sap.ui.getCore().byId("newPassWarningMsg").setVisible(true);
				return;
			}
		...
		onSelectShowPassSel: function(oEvent) {
			...
			sap.ui.getCore().byId("currentPass").setType(sap.m.InputType.Password);
